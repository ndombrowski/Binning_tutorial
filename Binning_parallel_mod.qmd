...
- Work with the contigs generated in the [Assembly tutorial](https://ndombrowski.github.io/Assembly_tutorial/). While these two tutorials are design to go hand it hand, this tutorial can stand on its own and you can either work on your own contigs, or contigs that were prepared and are provided via a repository.
...
:::{.callout-note}
The read mapping will take a few minutes, so have patience!

While you wait, we recommend that you scroll down and read through the explanations on what is happening when running BWA.
:::
...
For each metagenome the step `bwa mem`, generates a new folder, each contains three files:
...
While waiting, scroll down below and read about what samtools actually does and what the individual arguments do.
:::
...
- the unmapped reads (0 in our case because we discarded unmapped reads when running `samtools view -hb -F4` and we explain this in a bit more detail below)
...
Options used when running `samtools view` (the code we do not run right now, because it is relatively time consuming):
...
If you run this on your own, the coverage files, i.e. `1_mapping/NIOZ114/NIOZ114_depth_binsanity.cov`, should look something like this:
...
[MetaBAT](https://bitbucket.org/berkeleylab/metabat/src/master/)is a robust statistical framework for reconstructing genomes from metagenomic data [Kang et al., 2015](https://peerj.com/articles/1165/)
...
So in the next step we add some additional information and also make sure in the later steps that for each binning tool we run we generate bin names in a consistent manner. I.e. if some files end with `.fa` and others with `.fasta`, which is not ideal. We also want to remove extra symbols, such as dots, and add what binning tool we used in the file header. The final name will look something like ` mb_b1.fa`.
...
**A small reminder for using pipes in bash:**
A pipe is a form of redirection (transfer of standard output to some other destination). The code block below is doing the same as when running this code:
...
:::{.callout-note}
The software `rename` normally is installed on all Unix platforms but can work slightly differently depending on its source.  
...
[CONCOCT](https://github.com/BinPro/CONCOCT) is a program for unsupervised binning of metagenomic contigs by using nucleotide composition, coverage data in multiple samples and linkage data from paired end reads [(Alneberg et al., 2014)](https://www.nature.com/articles/nmeth.3103)
...
[MaxBin](https://sourceforge.net/projects/maxbin/) is an automated binning method that recovers individual genomes from metagenomes using an expectation-maximization algorithm [(Wu et al., 2014)](https://microbiomejournal.biomedcentral.com/articles/10.1186/2049-2618-2-26)
...
- ensure all files end with the extension `fna`, which is more commonly used for MAGs
...
- When we check the file names, we also see that we have two times then name `mx_b002.contigs.fa` for NIOZ130 and NIOZ114 samples. Duplicated are not good and can easily create a confusion, so that is something else we want to change.
...
We can now see that the header looks something like this: `>NIOZ114_mb_b2-NIOZ114_sc48_1`. It includes the BinID separated with a `-` symbol from the contig ID. This is a general format we will use for everything we have and stick with using either `_` or `-`. The less clutter the easier most workflows will be.
...
[CheckM](https://ecogenomics.github.io/CheckM/): A tool for assessing the quality of microbial genomes recovered from isolates, single cells, and metagenomes [(Parks et al., 2015)](https://genome.cshlp.org/content/25/7/1043)
...
Especially since a lot of genes sit in operons, such as ribosomal proteins, this can mean that we over/underestimate genome completeness. Issues with the CheckM approach are also discussed [here](https://microbe.net/2017/12/13/why-genome-completeness-and-contamination-estimates-are-more-complicated-than-you-think/)
...
For now we will provide you with example files but feel free to revisit the actual code at a later point:
